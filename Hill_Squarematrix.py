#IMPLEMENTING HILL CIPHER ENCRYPTION/DECRYPTION USING SQUARE KEY MATRIX I.E (3X3) : 
...
def show_encrypt():
    key = 'helloalli'          #Key text that is converted to key matrix
    key_len = len(key)
...
def show_decrypt():
    ...
    k = np.array(convert_to_num(key))
    k = np.reshape(k, (-1, n)) #to reshape into square matrix
